	...
	function addServicio(idServicio,nombre){
		 //Variable para conocer el idServicio del Proveedor, esto para proveedores registrados
		 var idServicioP=$("#idServicio").val();
		 if(idServicioP!='' && idServicioP==idServicio)
			 $('#servicio').append("<option value='"+idServicio+"' selected>"+nombre +"</option>");
		 else
			 $('#servicio').append("<option value='"+idServicio+"'>"+nombre +"</option>");
	}
